...
class dataPacket:
     ...
     def addValues(self, reconstructed):
          ...
          if len(reconstructed) == 6:             #Checks to see if the incoming bytes are correct in size
...
def createEntryNodeTwo(conn, nodeData):
    ...
    sql = ''' INSERT INTO nodetwo(batteryVoltage,soilMoisture,soilTemperature,airMoisture,airTemperature)
              VALUES(?,?,?,?,?) '''
    cur = conn.cursor()
    cur.execute(sql, nodeData)
    conn.commit()
...
def createEntryNodeThree(conn, nodeData):
    ...
    sql = ''' INSERT INTO nodethree(batteryVoltage,soilMoisture,soilTemperature,airMoisture,airTemperature)
              VALUES(?,?,?,?,?) '''
    cur = conn.cursor()
    cur.execute(sql, nodeData)
    conn.commit()
...
def createEntryNodeFour(conn, nodeData):
    ...
    sql = ''' INSERT INTO nodefour(batteryVoltage,soilMoisture,soilTemperature,airMoisture,airTemperature)
              VALUES(?,?,?,?,?) '''
    cur = conn.cursor()
    cur.execute(sql, nodeData)
    conn.commit()
...
def createEntryNodeFive(conn, nodeData):
    ...
    sql = ''' INSERT INTO nodefive(batteryVoltage,soilMoisture,soilTemperature,airMoisture,airTemperature)
              VALUES(?,?,?,?,?) '''
    cur = conn.cursor()
    cur.execute(sql, nodeData)
    conn.commit()
...
while True:
     ...
     x = ser.read(24)    #Reads values from Serial Port
